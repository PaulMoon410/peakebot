    ...
    req.on("end", () => {
      try {
        ...
        if (!payload.messages || !Array.isArray(payload.messages)) {
          sendJson(res, 400, { error: "Invalid request: messages array required." });
          return;
        }
        ...
        proxyReq.on("error", (error) => {
          sendJson(res, 502, { error: `Llama server error: ${error.message}` });
        });

        proxyReq.write(JSON.stringify(llamaPayload));
        proxyReq.end();
      } catch (error) {
        sendJson(res, 400, { error: `Invalid JSON: ${error.message}` });
      }
